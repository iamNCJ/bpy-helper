...
def render_blender_scene_random_pl(
    ...
    for d in range(len(datasets)):
        ...
        for i in tqdm(range(Ns[d])):
            # camera
            m = look_at_to_c2w(eyes[i + offset], (0, 0, 0), (0, 0, 1))
...
if __name__ == '__main__':
    ...
    render_blender_scene_random_pl(
        r'./complex_ball.blend', r'./Complex_Ball_PL_500',
        512, 512, 500, 100, 100,
        30.0, 2.0 * scale, 7.0 * scale,
        (4.0 * scale, 5.0 * scale), (10.0, 90.0),
        (4.0 * scale, 5.0 * scale), (0.0, 60.0),
        pl_energy=2000,
        seed_view=0, seed_pl=1,
        N_samples=256,
        rescale_scene=1.0 / scale,
        fix_light=False,
        hard_shadow=True)
